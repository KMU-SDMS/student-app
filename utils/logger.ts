...
export const logger = {
  info(message: string, meta?: LogMeta) {
    ...
    if (externalHooks?.info) externalHooks.info({ level: 'info', message, meta });
  },
  warn(message: string, meta?: LogMeta) {
    if (__DEV__) console.warn(format('warn', message, meta));
    if (externalHooks?.warn) externalHooks.warn({ level: 'warn', message, meta });
  },
  error(message: string, error?: unknown, meta?: LogMeta) {
    ...
    if (externalHooks?.error) externalHooks.error({ level: 'error', message, error, meta });
  },
};
